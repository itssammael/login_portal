                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
                    ...
                    <div class="bg-white p-5 rounded-2xl shadow-xs border border-gray-100 hover:shadow-md transition">
                        ...
                        <p class="text-xs text-emerald-600 font-medium mt-1 flex items-center">
                            <span class="size-1.5 rounded-full bg-emerald-500 me-1.5 animate-pulse"></span>
                    ...
                    <div class="bg-white p-5 rounded-2xl shadow-xs border border-gray-100 hover:shadow-md transition">
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <div class="size-9 rounded-xl bg-purple-50 text-purple-600 flex items-center justify-center">
                                <svg class="size-5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M8.625 12a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 01-2.555-.337A5.972 5.972 0 015.41 20.97a5.969 5.969 0 01-.474-.065 4.48 4.48 0 00.978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25z" />
                                </svg>
                            </div>
                        </div>
                        <div class="text-2xl font-extrabold text-gray-900">{{ stats.total_conversations }}</div>
                        <p class="text-xs text-gray-500 font-medium mt-1">Direct message channels</p>
                    </div>

                    <!-- Total Messages Sent -->
                    <div class="bg-white p-5 rounded-2xl shadow-xs border border-gray-100 hover:shadow-md transition">
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Messages Sent</span>
                            <div class="size-9 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center">
                                <svg class="size-5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5" />
                                </svg>
                            </div>
                        </div>
                        <div class="text-2xl font-extrabold text-gray-900">{{ stats.total_messages }}</div>
                    ...
                    <div class="bg-white p-5 rounded-2xl shadow-xs border border-gray-100 hover:shadow-md transition">
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Suspended Users</span>
                        ...
                        <div class="text-2xl font-extrabold" :class="stats.banned_users > 0 ? 'text-amber-600' : 'text-gray-900'">
                            {{ stats.banned_users }}
                        </div>
                        <p class="text-xs text-gray-500 font-medium mt-1">Enforced suspensions</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div class="bg-white rounded-2xl shadow-xs border border-gray-100 p-6">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="font-bold text-gray-900 text-base">Active Chat Threads</h3>
                            <Link :href="route('admin.chats')" class="text-xs text-indigo-600 hover:text-indigo-800 font-semibold">
                                View all →
                            </Link>
                        </div>
                        ...
                        <div class="divide-y divide-gray-100">
                            <template v-if="recentConversations.length > 0">
                                <div
                                    ...
                                    :key="chat.id"
                                    class="py-3 flex items-center justify-between hover:bg-gray-50/50 rounded-xl px-2 transition"
                                >
                    ...
                    <div class="bg-white rounded-2xl shadow-xs border border-gray-100 p-6">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="font-bold text-gray-900 text-base">Recent Audit Trail</h3>
                            <Link :href="route('admin.audit-logs')" class="text-xs text-indigo-600 hover:text-indigo-800 font-semibold">
                                View all logs →
                            </Link>
                        </div>
                        ...
                        <div class="divide-y divide-gray-100">
                            <template v-if="recentLogs.length > 0">
                                <div
                                    ...
                                    :key="log.id"
                                    class="py-3 flex items-start justify-between text-xs hover:bg-gray-50/50 rounded-xl px-2 transition"
                                >
                                    <div>
                                        <p class="font-semibold text-gray-900">
                                            {{ log.admin?.name || 'Administrator' }}
                                            <span class="font-normal text-gray-600">performed</span>
                                            <span class="font-medium text-indigo-700"> {{ log.action }}</span>
                                        </p>
                                        <p v-if="log.details" class="text-gray-500 mt-0.5">
                                            {{ log.details.target_name ? 'Target: ' + log.details.target_name : '' }}
                                            {{ log.details.title ? 'Title: ' + log.details.title : '' }}
                                        </p>
                                    </div>
                                    <span class="text-gray-400 shrink-0 ml-2 whitespace-nowrap">
