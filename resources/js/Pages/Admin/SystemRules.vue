...
<template>
    <AppLayout title="Admin - System Rules Engine">
        <template #header>
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-gray-600 mt-0.5">Configure platform moderation rules, rate limits, security controls, and automated policies</p>
                </div>
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
                    <div class="bg-cream-200 p-5 rounded-2xl shadow-xs border border-cream-500/50 flex items-center space-x-4">
                        ...
                        <div>
                            <div class="text-xs text-gray-600 font-medium">Configured Rules</div>
                    ...
                    <div class="bg-cream-200 p-5 rounded-2xl shadow-xs border border-cream-500/50 flex items-center space-x-4">
                        ...
                        <div>
                            <div class="text-xs text-gray-600 font-medium">Active Enforcements</div>
                            <div class="text-2xl font-bold text-forest-800 mt-0.5">{{ stats.active_rules }}</div>
                        </div>
                    </div>

                    <div class="bg-cream-200 p-5 rounded-2xl shadow-xs border border-cream-500/50 flex items-center space-x-4">
                        <div class="p-3 bg-golden-100 text-golden-700 rounded-xl shrink-0">
                            <svg class="size-6" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" />
                            </svg>
                        </div>
                        <div>
                            <div class="text-xs text-gray-600 font-medium">Disabled Rules</div>
                            <div class="text-2xl font-bold text-golden-700 mt-0.5">{{ stats.inactive_rules }}</div>
                        </div>
                    </div>
                    ...
                    <div class="bg-cream-200 p-5 rounded-2xl shadow-xs border border-cream-500/50 flex items-center space-x-4">
                        ...
                        <div>
                            <div class="text-xs text-gray-600 font-medium">Rule Categories</div>
                ...
                <div class="bg-cream-200 p-4 rounded-2xl shadow-xs border border-cream-500/50 mb-6 space-y-4">
                    <!-- Category Tabs -->
                    <div class="flex items-center space-x-2 overflow-x-auto pb-1 border-b border-cream-500/30">
                        <button
                            @click="filterByCategory('')"
                            class="px-3.5 py-1.5 text-xs font-semibold rounded-xl transition whitespace-nowrap"
                            :class="!currentCategory ? 'bg-forest-900 text-white shadow-xs' : 'text-gray-700 hover:bg-cream-300'"
                        >
                            All Categories
                        </button>
                        <button
                            ...
                            class="px-3.5 py-1.5 text-xs font-semibold rounded-xl transition whitespace-nowrap"
                            :class="currentCategory === key ? 'bg-forest-900 text-white shadow-xs' : 'text-gray-700 hover:bg-cream-300'"
                        >
                            {{ label }}
                        </button>
                    </div>
                ...
                <div v-if="rules.length > 0" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    >
                        <div class="p-6">
                            ...
                            <div class="flex items-start justify-between">
                                <div>
                                    ...
                                    <code class="text-[11px] font-mono text-forest-900/70 bg-cream-100 px-2 py-0.5 rounded mt-1 inline-block border border-cream-500/30">
                                        {{ rule.key }}
                                    </code>
                                </div>
                                ...
                                <button
                                    ...
                                    :class="rule.is_active ? 'bg-forest-600' : 'bg-cream-400'"
                                >
                                    <span
                            ...
                            <div class="mt-4 pt-3 border-t border-cream-500/30 flex items-center justify-between text-xs">
                                <span class="font-semibold text-gray-500 uppercase tracking-wider text-[11px]">Rule Value / Limit</span>

                                <div>
                                    <span
                                        v-if="rule.rule_type === 'boolean'"
                                        class="inline-flex items-center px-2.5 py-1 rounded-lg font-bold text-xs"
                                        :class="rule.value === 'true' || rule.value === '1' ? 'bg-emerald-100 text-emerald-800 border border-emerald-300' : 'bg-rose-100 text-rose-800 border border-rose-300'"
                                    >
                                    ...
                                    <span
                                        ...
                                        class="inline-flex items-center px-2.5 py-1 rounded-lg font-mono font-bold bg-forest-100 text-forest-900 border border-forest-300 text-xs"
                                    >
                                        {{ rule.value }}
                                    </span>
                                    ...
                                    <span
                                        v-else
                                        class="inline-flex items-center px-2.5 py-1 rounded-lg font-mono text-xs bg-cream-100 text-gray-800 border border-cream-500/50"
                                    >
                                        {{ rule.value || 'None' }}
                                    </span>
                                </div>
                        ...
                        <!-- Card Footer -->
                        <div class="bg-cream-100/70 px-6 py-3 border-t border-cream-500/30 flex items-center justify-between">
                            <span class="text-[11px] text-gray-500 font-medium">
                                Priority Weight: <strong class="text-gray-800">{{ rule.priority }}</strong>
                            </span>

                            <div class="flex items-center space-x-2">
                                <button
                                    @click="openEditModal(rule)"
                                    class="px-3 py-1 text-xs font-semibold text-forest-900 hover:text-forest-950 hover:bg-cream-300 rounded-lg transition"
                                >
                                    Edit Rule
                                </button>
                                <button
                ...
                <!-- Empty State -->
                <div v-else class="bg-cream-200 p-12 rounded-2xl border border-cream-500/50 text-center">
                    <div class="size-12 rounded-full bg-cream-100 text-gray-400 flex items-center justify-center mx-auto mb-3">
                        <svg class="size-6" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 6h9.75M10.5 6a1.5 1.5 0 11-3 0m3 0a1.5 1.5 0 10-3 0M3.75 6H7.5m3 12h9.75m-9.75 0a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m-3.75 0H7.5m9-6h3.75m-3.75 0a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m-9.75 0h9.75" />
                        </svg>
                    </div>
                    <h3 class="text-base font-bold text-gray-900">No System Rules Found</h3>
                    <p class="text-xs text-gray-600 mt-1">No operational rules match the active category filter or search criteria.</p>
                </div>
            </div>
        </div>

        <!-- Create / Edit System Rule Modal -->
        <div v-if="showModal" class="fixed inset-0 z-50 overflow-y-auto bg-black/40 backdrop-blur-xs flex items-center justify-center p-4">
            <div class="bg-cream-200 rounded-3xl max-w-2xl w-full shadow-2xl border border-cream-500/60 overflow-hidden transform transition-all">
                <!-- Modal Header -->
                <div class="px-6 py-5 border-b border-cream-500/40 flex items-center justify-between bg-cream-100/60">
                    <div>
                        <h3 class="text-lg font-bold text-gray-900">
                            {{ editingRule ? 'Edit Rule: ' + editingRule.name : 'Create New System Rule' }}
                        </h3>
                        <p class="text-xs text-gray-600 mt-0.5">Configure rule parameters, category scope, and active enforcement state</p>
                    </div>

                    <button @click="closeModal" class="text-gray-400 hover:text-gray-600 p-1 rounded-lg hover:bg-cream-300 transition">
                ...
                <form @submit.prevent="saveRule" class="p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input
                                ...
                                class="w-full px-3.5 py-2.5 bg-[#fffef9] border border-cream-500 focus:bg-white focus:border-forest-600 focus:ring-2 focus:ring-forest-200 rounded-xl text-sm transition text-gray-900"
                                required
                            />
                        ...
                        <div>
                            ...
                            <input
                                ...
                                class="w-full px-3.5 py-2.5 bg-[#fffef9] border border-cream-500 focus:bg-white focus:border-forest-600 focus:ring-2 focus:ring-forest-200 rounded-xl text-sm transition disabled:opacity-60 text-gray-900"
                            />
                    ...
                    <div>
                        ...
                        <div v-if="form.rule_type === 'boolean'" class="flex items-center space-x-4 bg-cream-100 p-3 rounded-xl border border-cream-500/40">
                            <label class="flex items-center space-x-2 cursor-pointer text-xs font-bold text-gray-800">
                                <input type="radio" v-model="form.value" value="true" class="text-forest-900 focus:ring-forest-600 border-cream-500" />
                                <span>Enabled (true)</span>
                            </label>
                            <label class="flex items-center space-x-2 cursor-pointer text-xs font-bold text-gray-800">
                                <input type="radio" v-model="form.value" value="false" class="text-forest-900 focus:ring-forest-600 border-cream-500" />
                        ...
                        <textarea
                            ...
                            class="w-full px-3.5 py-2.5 bg-[#fffef9] border border-cream-500 focus:bg-white focus:border-forest-600 focus:ring-2 focus:ring-forest-200 rounded-xl text-sm transition font-mono text-gray-900"
                        ></textarea>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-center pt-2">
                        ...
                        <div class="flex items-center space-x-3 md:mt-5">
                            <input
                                ...
                                id="is_active_toggle"
                                type="checkbox"
                                class="size-4 text-forest-900 focus:ring-forest-600 border-cream-500 rounded"
                            />
                    ...
                    <!-- Modal Actions -->
                    <div class="pt-4 border-t border-cream-500/30 flex items-center justify-end space-x-3">
                        <button
                            type="button"
                            @click="closeModal"
        ...
        <div v-if="showDeleteConfirm" class="fixed inset-0 z-50 overflow-y-auto bg-black/40 backdrop-blur-xs flex items-center justify-center p-4">
            <div class="bg-cream-200 rounded-3xl max-w-md w-full shadow-2xl border border-cream-500/60 p-6 text-center">
